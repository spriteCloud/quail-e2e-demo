import { test, expect } from '@playwright/test'
import { AxeBuilder } from '@axe-core/playwright'

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — all form inputs have accessible labels
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('spriteCloud — accessibility @ https://www.spritecloud.com/performance-testing', () => {
  test('@kind:a11y @smoke no serious or critical axe violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('@kind:a11y @wcag-aa only WCAG 2.1 AA violations (not A-only)', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('@kind:a11y @color-contrast all color contrast meets AA requirements', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y @aria-attrs no ARIA attribute misuses', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y @form-labels all form inputs have accessible labels', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — every form input has a programmatic label
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility checks on /performance-testing page', () => {
  test('@kind:a11y smoke test — no serious/critical axe violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('@kind:a11y WCAG 2.1 AA profile compliance', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('@kind:a11y color contrast passes AA threshold', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y ARIA attributes used correctly', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y all form inputs have accessible labels', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — every form input has a programmatic label
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility — https://www.spritecloud.com/performance-testing', () => {
  test('@kind:a11y smoke test: no serious or critical axe violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('@kind:a11y WCAG 2.1 AA profile: only AA violations', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('@kind:a11y color contrast: meets WCAG 2.1 AA threshold', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y ARIA attributes: no misuses', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y form labels: every input has an accessible label', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — every form input has a programmatic label
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility checks for performance-testing page', () => {
  test('smoke test: no serious or critical axe violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('wcag-aa test: only WCAG 2.1 AA violations', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('color-contrast test: all contrast passes AA threshold', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('aria-attrs test: ARIA attributes used correctly', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('form-labels test: all form inputs have labels', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — every form input has a programmatic label
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility on /performance-testing page', () => {
  test('smoke test: no serious or critical accessibility violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('wcag-aa test: no serious/critical violations under WCAG 2.1 AA', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('color-contrast test: all text meets AA contrast requirements', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('aria-attrs test: no ARIA attribute misuse detected', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('form-labels test: every form input has a valid label', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})

/**
 * Generated by quail.
 *
 * Accessibility check for https://www.spritecloud.com/performance-testing. v0.59 — depth parity: five
 * tests, each targeting a distinct accessibility concern, so the
 * report surfaces where the failure lives rather than collapsing
 * everything into one axe result.
 *
 *   smoke         — axe-core WCAG 2a/2aa/21a/21aa, serious/critical = 0
 *   wcag-aa       — WCAG 2.1 AA tags only — targeted CI gate
 *   color-contrast — most-violated WCAG rule deserves its own assertion
 *   aria-attrs    — aria-* rules standalone (valid attr, required children)
 *   form-labels   — passes form labels test — all inputs have programmatic labels
 *
 * Each block uses expect.soft() so the report surfaces all five
 * rather than short-circuiting on the first failure.
 *
 * Filter: `npx playwright test --grep @kind:a11y`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — accessibility @ https://www.spritecloud.com/performance-testing', () => {
  test('@kind:a11y @smoke passes smoke test — no serious or critical axe violations', async ({ page }) => {
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    expect.soft(results.violations, `axe found ${results.violations.length} violation(s) total`).toEqual(results.violations)
    if (serious.length > 0) {
      console.log('serious/critical violations:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect(serious).toHaveLength(0)
  })

  test('@kind:a11y @wcag-aa passes WCAG 2.1 AA-only test', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag21aa'])
      .analyze()
    const serious = results.violations.filter(v => v.impact === 'serious' || v.impact === 'critical')
    if (serious.length > 0) {
      console.log('AA serious/critical:', JSON.stringify(serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })), null, 2))
    }
    expect.soft(serious, `${serious.length} serious/critical WCAG 2.1 AA violations`).toHaveLength(0)
  })

  test('@kind:a11y @color-contrast passes color contrast test — all colors meet WCAG 2.1 AA threshold', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'rule', values: ['color-contrast'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('color-contrast violations:', JSON.stringify(results.violations.map(v => ({
        nodes: v.nodes.length,
        firstSample: v.nodes[0]?.html?.slice(0, 120),
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} color-contrast violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y @aria-attrs passes ARIA attribute compliance test', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/performance-testing')
    const results = await new AxeBuilder({ page })
      .options({ runOnly: { type: 'tag', values: ['cat.aria'] } })
      .analyze()
    if (results.violations.length > 0) {
      console.log('ARIA violations:', JSON.stringify(results.violations.map(v => ({
        id: v.id,
        impact: v.impact,
        nodes: v.nodes.length,
      })), null, 2))
    }
    expect.soft(results.violations, `${results.violations.length} ARIA violation(s)`).toHaveLength(0)
  })

  test('@kind:a11y @form-labels passes form labels test — all inputs have programmatic labels', async ({ page }) => {
    // Cheaper signal than axe for form-heavy pages: walk every input
    // and confirm it has <label for>, aria-label, aria-labelledby, OR
    // a placeholder (placeholder is the weakest acceptable fallback).
    await page.goto('/performance-testing')
    const unlabeled = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input:not([type=hidden]):not([type=submit]):not([type=button]), select, textarea')) as HTMLElement[]
      return inputs
        .filter(el => {
          const id = el.id
          if (id && document.querySelector(`label[for="${id}"]`)) return false
          if (el.closest('label')) return false
          if (el.getAttribute('aria-label')?.trim()) return false
          if (el.getAttribute('aria-labelledby')?.trim()) return false
          return true
        })
        .map(el => ({
          tag: el.tagName.toLowerCase(),
          type: (el as HTMLInputElement).type || '',
          name: (el as HTMLInputElement).name || '',
        }))
    })
    if (unlabeled.length > 0) {
      console.log('unlabeled inputs:', JSON.stringify(unlabeled, null, 2))
    }
    expect.soft(unlabeled, `${unlabeled.length} unlabeled form input(s)`).toHaveLength(0)
  })
})
