 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — fuzz tests with negative inputs', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
  })
  ...
  test('@fuzz @negative input "email-2" handles edge payloads', async ({ page }) => {
    ...
    await field.fill('')
    await expect(field).toHaveValue('')
    // Very long input (5000 chars): field should remain responsive.
    const long = 'x'.repeat(5000)
    await field.fill(long)
    const persisted = await field.inputValue()
    expect(persisted.length).toBeGreaterThan(0)
    // Special-character payload: HTML injection attempt. Must be safely escaped or ignored
    // (handled by the pageErrors fixture in _fixtures.ts).
    await field.fill('\'><script>alert(1)</script>')
    await expect(field).toBeAttached()
  })
