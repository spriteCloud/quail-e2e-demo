...
// v0.22 — cheap pre-check: if the failing name isn't a substring of
// the current DOM, the target isn't on this page (composer
// hallucination, or the journey landed on the wrong step). Skip the
// LLM call — no candidate could possibly resolve.
function pageHasName(dom: string, name: string): boolean {
  if (!name) return true
  return dom.toLowerCase().includes(name.toLowerCase())
}
...
async function withHeal<T>(
  ...
  try {
    ...
    if (candidates === undefined) {
      ...
      // v0.22 — DOM pre-check. If the failing name isn't anywhere in
      // the DOM, no CSS selector can rescue it. Cache the negative
      // result so sibling tests hitting the same broken locator skip
      // the LLM call too.
      if (!pageHasName(dom, args.name || '')) {
        HEAL_CACHE.set(key, [])
        await logHealed({
          at: new Date().toISOString(),
          op, args,
          outcome: 'skipped-not-on-page',
          error: err.message,
        })
