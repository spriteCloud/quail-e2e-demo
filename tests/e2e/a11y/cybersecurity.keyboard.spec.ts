...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/cybersecurity', () => {
  test('@kind:keyboard @smoke verify tab navigation cycles through the first 10 interactive elements', async ({ page }) => {
    await page.goto('/cybersecurity')
  ...
  test('@kind:keyboard @focus-indicator verify the first tab-focused element shows a visible focus outline or shadow', async ({ page }) => {
    await page.goto('/cybersecurity')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @escape-dismiss verify pressing Escape dismisses an open dialog and returns focus to its trigger', async ({ page }) => {
    await page.goto('/cybersecurity')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
  ...
  test('@kind:keyboard @enter-space verify Enter triggers links or button actions when focused', async ({ page }) => {
    await page.goto('/cybersecurity')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap verify Tab from the last interactive element either wraps or exits page navigation', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/cybersecurity')
