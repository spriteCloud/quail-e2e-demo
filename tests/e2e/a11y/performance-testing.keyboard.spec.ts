...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/performance-testing', () => {
  ...
  test('@kind:keyboard @escape-dismiss Pressing Escape closes the visible dialog and restores focus to the trigger', async ({ page }) => {
    await page.goto('/performance-testing')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap Pressing Tab after the last focusable element moves focus forward or wraps to the start', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/performance-testing')
