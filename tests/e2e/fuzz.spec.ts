 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — fuzz testing for edge-case and invalid inputs', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/')
  })

  test('fuzz test: email field accepts empty, very long, and malicious payloads without crashing', async ({ page }) => {
    const field = page.getByPlaceholder('Your email address').first()
    // Empty value: should be accepted by the field itself (validation
    // surfaces only on submit).
    await field.fill('')
