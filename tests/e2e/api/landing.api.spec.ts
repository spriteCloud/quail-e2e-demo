...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — API contract for submission endpoint', () => {
  ...
  test('@kind:api @negative empty form data returns client error (4xx)', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {},
    })
    // 4xx is the contract; 5xx means the server crashed on empty input
  ...
  test('@kind:api @negative invalid email format returns client error (4xx)', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'not-an-email',
      },
  ...
  test('@kind:api @negative very large payload doesn’t cause server error (5xx)', async ({ request }) => {
    const huge = 'a'.repeat(50_000)
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': huge,
  ...
  test('@kind:api @negative unicode characters in input don’t cause server error (5xx)', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'café-niño-用户-🎉-test@example.com',
      },
  ...
  test('@kind:api @negative SQL-like input doesn’t crash server (5xx) or leak details', async ({ request }) => {
    // Tests the server's input sanitization. The contract: MUST NOT
    // 5xx and MUST NOT echo back unescaped fragments.
    const payload = "'; DROP TABLE users; --"
    const response = await request.get('https://www.spritecloud.com/', {
  ...
  test('@kind:api @negative XSS-style input is sanitized or rejected', async ({ request }) => {
    const payload = '<script>window.__rqXSS=1</script>'
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': payload,
  ...
  test('@kind:api @negative null-byte in input doesn’t crash server (5xx)', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'value\x00malicious',
      },
  ...
  test('@kind:api @negative high-frequency requests remain stable or are rate-limited', async ({ request }) => {
    // 10 concurrent identical requests. Either the server rate-limits
    // (429) — fine — or it serves all 10 without 5xx-ing.
    const body = {
      'email-2': 'test@example.com',
