 ...
 * Generated by quail.
 *
 * Accessibility landmark structure for https://www.spritecloud.com/guides. v0.59 — depth
...
test.describe('spriteCloud — a11y landmarks @ https://www.spritecloud.com/guides', () => {
 ...
 *   heading-hierarchy   — headings follow sequential hierarchy without skipping levels (no h1→h3 jumps)
 *   landmark-names      — duplicate landmarks carry accessible names
 *   skip-link           — "skip to content" link present (WCAG 2.4.1)
 *
 * Filter: `npx playwright test --grep @kind:a11y-landmarks`
...
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/guides', () => {
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow sequential hierarchy without skipping levels', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/guides')
  ...
  test('@kind:a11y-landmarks @landmark-names duplicate landmarks include accessible names for distinction', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/guides')
  ...
  test('@kind:a11y-landmarks @skip-link "skip to content" link present as required by WCAG 2.4.1', async ({ page }) => {
    // The first focusable should either be a skip link OR there must
    // be an in-page anchor to #main / #content / [role=main]. WCAG
    // 2.4.1 ("Bypass Blocks") requires it for keyboard / SR users.
    await page.goto('/guides')
