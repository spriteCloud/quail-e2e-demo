...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/contact', () => {
  test('@kind:keyboard @smoke: Tab cycles through first 10 focusable elements', async ({ page }) => {
    await page.goto('/contact')
  ...
  test('@kind:keyboard @focus-indicator: First tab-focused element has visible focus indicator', async ({ page }) => {
    await page.goto('/contact')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @escape-dismiss: Escape closes visible dialog and returns focus to trigger', async ({ page }) => {
    await page.goto('/contact')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap: Tab from last focusable wraps or exits page (no trap)', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/contact')
