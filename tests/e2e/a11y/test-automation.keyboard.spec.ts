...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/test-automation', () => {
  ...
  test('@kind:keyboard @focus-indicator first Tab-focused element shows visible focus outline or shadow', async ({ page }) => {
    await page.goto('/test-automation')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @enter-space Enter triggers action for focused links and buttons', async ({ page }) => {
    await page.goto('/test-automation')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap Tabbing past the last interactive element should wrap or exit', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/test-automation')
