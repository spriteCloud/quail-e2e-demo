...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/test-automation', () => {
  test('@kind:keyboard @smoke navigates with Tab through the first 10 interactive elements', async ({ page }) => {
    await page.goto('/test-automation')
    ...
    expect.soft(reached, `Tab navigation reached only ${reached} of ${max} interactive elements`).toBeGreaterThanOrEqual(Math.floor(max * 0.7))
  })

  test('@kind:keyboard @focus-indicator the first element focused with Tab has a visible outline or box-shadow', async ({ page }) => {
    await page.goto('/test-automation')
    await page.keyboard.press('Tab')
    ...
    expect.soft(hasIndicator, 'the first element focused using Tab lacks a visible focus indicator').toBe(true)
  })

  test('@kind:keyboard @escape-dismiss pressing Escape closes an open dialog and restores focus to its trigger', async ({ page }) => {
    await page.goto('/test-automation')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
    ...
    await page.keyboard.press('Escape')
    const stillOpen = await dialog.isVisible().catch(() => false)
    expect.soft(stillOpen, 'pressing Escape should close the dialog').toBe(false)
  })
  ...
  test('@kind:keyboard @enter-space pressing Enter on a focused link or button triggers its action', async ({ page }) => {
    ...
    if (!activated) {
      // soft so SPAs that consume the click via JS don't fail —
      // a real navigation would have flipped the flag.
      expect.soft(activated, 'pressing Enter on a focused link should produce a visible effect such as navigation or JS-triggered action').toBe(true)
    }
  })

  test('@kind:keyboard @no-trap pressing Tab after the last interactive element exits or loops back', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/test-automation')
    ...
    expect.soft(trapped, `pressing Tab after the last interactive element should either loop to the first or move focus outside the page; observed focus stayed on ${firstActiveTag}`).toBe(false)
  })
})
