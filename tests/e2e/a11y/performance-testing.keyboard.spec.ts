...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/performance-testing', () => {
  test('@kind:keyboard @smoke navigate to the first 10 focusable elements using Tab', async ({ page }) => {
    await page.goto('/performance-testing')
  ...
  test('@kind:keyboard @no-trap pressing Tab after the last focusable element wraps back to the first or exits the page', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/performance-testing')
