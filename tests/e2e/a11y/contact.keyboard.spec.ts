...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/contact', () => {
  ...
  test('@kind:keyboard @focus-indicator the initially focused element has a visible focus indicator', async ({ page }) => {
    await page.goto('/contact')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @enter-space Enter activates the currently focused link or button', async ({ page }) => {
    await page.goto('/contact')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/contact', () => {
  test('@kind:keyboard @smoke tab through the first 10 focusables', async ({ page }) => {
    ...
    expect.soft(reached, `Tabbing reached only ${reached} of ${max} focusable elements`).toBeGreaterThanOrEqual(Math.floor(max * 0.7))
  })
  ...
  test('@kind:keyboard @focus-indicator first focused element has a visible focus indicator', async ({ page }) => {
    ...
    expect.soft(hasIndicator, 'first element focused via Tab lacks a visible focus indicator').toBe(true)
  })
  ...
  test('@kind:keyboard @enter-space Enter activates focused links/buttons', async ({ page }) => {
    ...
    if (!activated) {
      // soft so SPAs that consume the click via JS don't fail —
      // a real navigation would have flipped the flag.
      expect.soft(activated, 'Pressing Enter on a focused link produced no observable effect').toBe(true)
    }
  })
