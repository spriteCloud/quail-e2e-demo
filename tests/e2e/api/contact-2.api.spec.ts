 ...
 *   - this file (*.api.spec.ts): "POST https://www.spritecloud.com/contact still accepts our shape"
 *
 * Endpoint:     https://www.spritecloud.com/contact
 * Method:       get
...
test.describe('Spritecloud — API contract @ https://www.spritecloud.com/contact', () => {

  test('@kind:api @smoke happy: realistic body returns 2xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/contact', {
      form: {
        'first_name': 'Alex',
        'last_name': 'Tester',
  ...
  test('@kind:api @negative unicode payload does not 5xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/contact', {
      form: {
        'first_name': 'café-niño-用户-🎉-Alex',
        'last_name': 'café-niño-用户-🎉-Tester',
        'email': 'café-niño-用户-🎉-test@example.com',
        'company': 'café-niño-用户-🎉-company-value',
        'service': 'café-niño-用户-🎉-3.5',
        'message': 'café-niño-用户-🎉-35',
      },
    })
    expect(response.status()).toBeLessThan(500)
  })
  ...
  test('@kind:api @negative null-byte injection does not 5xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/contact', {
      form: {
        'first_name': 'value\x00malicious',
        'last_name': 'value\x00malicious',
        'email': 'value\x00malicious',
        'company': 'value\x00malicious',
        'service': 'value\x00malicious',
        'message': 'value\x00malicious',
      },
    })
    expect(response.status()).toBeLessThan(500)
  })
  ...
  test('@kind:api @negative rapid burst stays healthy or is rate-limited', async ({ request }) => {
    ...
    const responses = await Promise.all(
      Array.from({ length: 10 }, () => request.get('https://www.spritecloud.com/contact', { form: body }))
    )
