...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — Accessibility checks for site', () => {
  test('@kind:a11y Smoke test: no serious or critical WCAG violations', async ({ page }) => {
    await page.goto('/')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
  ...
  test('@kind:a11y Color contrast conforms to AA requirements', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/')
