 ...
 * baseline axe scan does NOT enforce:
 *
 *   smoke               — single <main>, single <h1>, at least one <nav>
 *   no-aria-hidden      — no interactive elements inside aria-hidden sections
 *   heading-hierarchy   — headings follow sequential level order (no skipped levels like h1→h3)
 *   landmark-names      — duplicate landmarks each have an accessible name
 *   skip-link           — "skip to content" link present (WCAG 2.4.1)
 *
 * Filter: `npx playwright test --grep @kind:a11y-landmarks`
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/performance-testing', () => {
  test('@kind:a11y-landmarks @smoke: one <main>, one <h1>, ≥1 <nav>', async ({ page }) => {
    await page.goto('/performance-testing')
  ...
  test('@kind:a11y-landmarks @negative: no interactive elements in aria-hidden regions', async ({ page }) => {
    await page.goto('/performance-testing')
    const hiddenFocusables = await page.locator('[aria-hidden="true"]').locator('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])').count()
    expect.soft(hiddenFocusables, `${hiddenFocusables} focusable elements inside aria-hidden — confusing for screen readers`).toBe(0)
  })

  test('@kind:a11y-landmarks @heading-hierarchy: headings ascend/descend sequentially', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/performance-testing')
  ...
  test('@kind:a11y-landmarks @landmark-names: repeated landmarks have unique accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/performance-testing')
