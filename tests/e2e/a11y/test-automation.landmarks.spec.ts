 ...
 *   heading-hierarchy   — headings follow sequential order (no skips) (no h1→h3 jumps)
 *   landmark-names      — duplicate landmarks carry accessible names
 *   skip-link           — "skip to content" link present (WCAG 2.4.1)
 *
 * Filter: `npx playwright test --grep @kind:a11y-landmarks`
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/test-automation', () => {
  test('@kind:a11y-landmarks @smoke one main element, one h1, at least one nav', async ({ page }) => {
    await page.goto('/test-automation')
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow sequential order (no skips)', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/test-automation')
