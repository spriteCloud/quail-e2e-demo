 ...
 * Generated by quail.
 *
 * Keyboard-only navigation for https://www.spritecloud.com/devops. v0.59 — depth parity:
...
test.describe('spriteCloud — keyboard navigation @ https://www.spritecloud.com/devops', () => {
...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/devops', () => {
  ...
  test('@kind:keyboard @escape-dismiss Escape closes visible dialog and returns focus', async ({ page }) => {
    await page.goto('/devops')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap Tab from last focusable wraps or exits page', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/devops')
...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/devops', () => {
  ...
  test('@kind:keyboard @enter-space pressing Enter activates focused links and buttons', async ({ page }) => {
    await page.goto('/devops')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap pressing Tab from the last focusable wraps or exits the page', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/devops')
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/devops', () => {
  test('@kind:keyboard @smoke navigate through the first 10 interactive elements with Tab', async ({ page }) => {
    await page.goto('/devops')
  ...
  test('@kind:keyboard @focus-indicator the first element focused with Tab shows a visible focus ring', async ({ page }) => {
    await page.goto('/devops')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @enter-space Enter triggers actions on focused links and buttons', async ({ page }) => {
    await page.goto('/devops')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
  ...
  test('@kind:keyboard @no-trap Tab after the last interactive element moves focus forward or cycles back', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/devops')
