 ...
 * Generated by quail.
 *
 * Keyboard-only navigation for https://www.spritecloud.com/case-studies. v0.59 — depth parity:
...
test.describe('spriteCloud — keyboard navigation @ https://www.spritecloud.com/case-studies', () => {
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/case-studies', () => {
  test('@kind:keyboard @smoke tabs through first 10 interactive elements', async ({ page }) => {
    await page.goto('/case-studies')
  ...
  test('@kind:keyboard @escape-dismiss Escape closes an open dialog and returns focus to trigger', async ({ page }) => {
    await page.goto('/case-studies')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
 ...
 *   no-trap         — Tab from the last focusable wraps or exits (no focus trap)
 *
 * Filter: `npx playwright test --grep @kind:keyboard`
 */
test.describe.configure({ mode: 'parallel' })
