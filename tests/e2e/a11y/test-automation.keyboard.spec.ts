...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/test-automation', () => {
  ...
  test('@kind:keyboard @focus-indicator first focused element has a visible focus indicator', async ({ page }) => {
    ...
    expect.soft(hasIndicator, 'first element focused with Tab has no visible focus indicator').toBe(true)
  })
  ...
  test('@kind:keyboard @no-trap Tab from the last focusable wraps or exits the page (no focus trap)', async ({ page }) => {
    ...
    expect.soft(trapped, `Tab past the last focusable should wrap or exit focus; instead focus stayed on ${firstActiveTag}→${secondActiveTag}`).toBe(false)
  })
})
