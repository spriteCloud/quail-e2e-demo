...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud contact page accessibility checks', () => {
  test('no serious or critical accessibility violations per axe-core', async ({ page }) => {
    await page.goto('/contact')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
  ...
  test('compliance with WCAG 2.1 AA requirements', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/contact')
  ...
  test('text and background color contrast meets WCAG AA standards', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/contact')
  ...
  test('ARIA attributes used correctly and consistently', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/contact')
...
test.describe('Spritecloud — accessibility @ https://www.spritecloud.com/contact', () => {
  test('@kind:a11y @smoke passes smoke test (no serious/critical axe violations)', async ({ page }) => {
    await page.goto('/contact')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
  ...
  test('@kind:a11y @wcag-aa passes WCAG 2.1 AA profile check', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/contact')
  ...
  test('@kind:a11y @color-contrast meets WCAG 2.1 AA color contrast thresholds', async ({ page }) => {
    // The single most-violated WCAG rule in production. Worth its own
    // assertion so the report surfaces contrast separately from the
    // rest of the axe bag.
    await page.goto('/contact')
  ...
  test('@kind:a11y @aria-attrs passes ARIA attribute compliance check', async ({ page }) => {
    // Catch the common ARIA mis-uses (invalid attr names, required
    // children missing, valid roles for elements). Separate gate so
    // teams can distinguish a wiring bug from a visual issue.
    await page.goto('/contact')
