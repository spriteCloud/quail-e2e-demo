...
test.describe('Spritecloud — security headers on https://www.spritecloud.com', () => {
  test('security headers smoke test: baseline headers are present', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com')
    expect(response.status(), 'origin should respond with a valid HTTP status').toBeLessThan(500)
    const headers = response.headers()
    ...
    const xfo = headers['x-frame-options']
    const cspFrameAncestors = /frame-ancestors/i.test(csp)
    expect.soft(xfo || cspFrameAncestors, 'no clickjacking protection found (X-Frame-Options or CSP frame-ancestors)').toBeTruthy()
