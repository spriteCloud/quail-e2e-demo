 ...
 * Generated by quail.
 *
 * Accessibility landmark structure for https://www.spritecloud.com/. v0.59 — depth
 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility landmark structure at https://www.spritecloud.com/', () => {
  test('@kind:a11y-landmarks @smoke page has exactly one main region, one h1, and at least one nav', async ({ page }) => {
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow sequential order without skips', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @landmark-names repeated landmarks include accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/')
...
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/', () => {
  ...
  test('@kind:a11y-landmarks negative-test: no focusable elements in aria-hidden regions', async ({ page }) => {
    await page.goto('/')
    const hiddenFocusables = await page.locator('[aria-hidden="true"]').locator('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])').count()
    expect.soft(hiddenFocusables, `${hiddenFocusables} focusable elements inside aria-hidden — confusing for screen readers`).toBe(0)
  })
  ...
  test('@kind:a11y-landmarks landmark-names: repeated landmarks have accessible distinguishing names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/')
 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility landmarks on https://www.spritecloud.com/', () => {
  test('smoke test: exactly one main, h1, and at least one nav', async ({ page }) => {
    await page.goto('/')
  ...
  test('negative test: no focusable elements inside aria-hidden regions', async ({ page }) => {
    await page.goto('/')
    const hiddenFocusables = await page.locator('[aria-hidden="true"]').locator('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])').count()
    expect.soft(hiddenFocusables, `${hiddenFocusables} focusable elements inside aria-hidden — confusing for screen readers`).toBe(0)
  })

  test('heading hierarchy: headings must follow sequential order (no skipped levels)', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/')
  ...
  test('landmark names: multiple landmarks must have distinct accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/')
  ...
  test('skip link: “skip to content” link is present (WCAG 2.4.1)', async ({ page }) => {
    // The first focusable should either be a skip link OR there must
    // be an in-page anchor to #main / #content / [role=main]. WCAG
    // 2.4.1 ("Bypass Blocks") requires it for keyboard / SR users.
    await page.goto('/')
 ...
 *   heading-hierarchy   — headings follow sequential hierarchy (no skippinglevels) (no h1→h3 jumps)
 *   landmark-names      — duplicate landmarks carry accessible names
 *   skip-link           — "skip to content" link present (WCAG 2.4.1)
 *
 * Filter: `npx playwright test --grep @kind:a11y-landmarks`
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/', () => {
  test('@kind:a11y-landmarks @smoke exactly one <main>, one <h1>, and at least one <nav>', async ({ page }) => {
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @heading-hierarchy headings follow sequential hierarchy (no skippinglevels)', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @landmark-names duplicate landmarks each have accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @skip-link "skip to content" link or in-page anchor present (WCAG 2.4.1)', async ({ page }) => {
    // The first focusable should either be a skip link OR there must
    // be an in-page anchor to #main / #content / [role=main]. WCAG
    // 2.4.1 ("Bypass Blocks") requires it for keyboard / SR users.
    await page.goto('/')
