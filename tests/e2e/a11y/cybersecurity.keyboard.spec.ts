 ...
 * cannot enforce statically:
 *
 *   smoke           — Tab navigates through the first 10 focusable elements
 *   focus-indicator — each focusable element has a visible focus indicator
 *   escape-dismiss  — Escape dismisses a visible dialog and returns focus
 *   enter-space     — Enter activates links and buttons; Space activates buttons
 *   no-trap         — Tab from the last focusable element wraps or exits (no focus trap)
 *
 * Filter: `npx playwright test --grep @kind:keyboard`
 */
test.describe.configure({ mode: 'parallel' })
