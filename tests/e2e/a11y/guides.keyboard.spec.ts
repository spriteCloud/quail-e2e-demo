...
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/guides', () => {
  ...
  test('@kind:keyboard @focus-indicator First focused element displays a visible focus indicator', async ({ page }) => {
    await page.goto('/guides')
    await page.keyboard.press('Tab')
  ...
  test('@kind:keyboard @enter-space Enter key activates focused links or buttons', async ({ page }) => {
    await page.goto('/guides')
    // Find the first interactive control of each type.
    const link = page.locator('a[href]:not([href^="#"]):not([href^="mailto:"]):not([href^="tel:"])').first()
    if (await link.count() === 0) test.skip()
