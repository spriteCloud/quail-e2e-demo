...
test.describe('Spritecloud — accessibility @ https://www.spritecloud.com/contact', () => {
  test('@kind:a11y @smoke no serious or critical accessibility violations detected by axe', async ({ page }) => {
    await page.goto('/contact')
    const results = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa'])
      .analyze()
  ...
  test('@kind:a11y @wcag-aa WCAG 2.1 AA compliance verified', async ({ page }) => {
    // Targeted gate: many teams require AA compliance specifically.
    // Running with just the wcag21aa tag lets the report distinguish
    // AA-required failures from A-required ones in a single grep.
    await page.goto('/contact')
