 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — security headers on www.spritecloud.com', () => {
  test('baseline security headers are present', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com')
    expect(response.status(), 'server should respond').toBeLessThan(500)
    const headers = response.headers()

    expect.soft(headers['strict-transport-security'], 'Strict-Transport-Security header missing').toBeTruthy()
    const csp = headers['content-security-policy'] || ''
 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — security headers', () => {
  test('Baseline security headers are present', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com')
    expect(response.status(), 'origin should respond').toBeLessThan(500)
    const headers = response.headers()
...
test.describe('Spritecloud — security headers for https://www.spritecloud.com', () => {
  test('Security headers smoke test: baseline headers present', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com')
    expect(response.status(), 'Origin should respond').toBeLessThan(500)
    const headers = response.headers()
 ...
 */
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — Security headers check at https://www.spritecloud.com', () => {
  test('Baseline security headers are present (@smoke)', async ({ request }) => {
    ...
    expect.soft(headers['strict-transport-security'], 'Strict-Transport-Security header is missing').toBeTruthy()
    const csp = headers['content-security-policy'] || ''
    expect.soft(csp, 'Content-Security-Policy header is missing').toBeTruthy()
    const xfo = headers['x-frame-options']
    const cspFrameAncestors = /frame-ancestors/i.test(csp)
    expect.soft(xfo || cspFrameAncestors, 'Missing clickjacking protection (neither X-Frame-Options nor CSP frame-ancestors found)').toBeTruthy()
