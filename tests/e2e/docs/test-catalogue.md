...
Demo baseline is scoped to journeys + heal + fuzz. Companion axes
(a11y, responsive, perf, security, health, observability, contract,
i18n) are pruned from the repo but remain available via
`QUAIL_KINDS=a11y,responsive,perf,...` when re-generating locally or
on a real customer suite. See `DEMO.md` for the demo scope rationale.
