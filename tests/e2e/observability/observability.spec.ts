...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — observability headers on www.spritecloud.com', () => {
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — observability headers smoke test for https://www.spritecloud.com', () => {
  test('origin emits at least one observability header in response', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com')
    const headers = response.headers()
    const signals = {
      'x-request-id': headers['x-request-id'] || '',
