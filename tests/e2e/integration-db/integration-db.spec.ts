...
test.describe('Spritecloud — integration: DB scaffold (skipped)', () => {
  ...
  test.skip('integration: DB transaction rollback — fails cleanly on error', async ({ request }) => {
    // When wired:
    //   1. Begin transaction
    //   2. POST with one valid + one invalid record
    //   3. Confirm no rows committed (full rollback)
  ...
  test.skip('integration: DB connection pool — concurrent requests stay within limits', async ({ request }) => {
    // When wired:
    //   1. Issue N concurrent reads where N > pool.max
    //   2. Confirm all N return < 500
    //   3. Confirm pool.metrics.waiting drops to 0 within 30s
