 ...
 * Generated by quail.
 *
 * Keyboard-only navigation for https://www.spritecloud.com/guides. v0.59 — depth parity:
...
test.describe('spriteCloud — keyboard navigation @ https://www.spritecloud.com/guides', () => {
...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — keyboard navigation @ https://www.spritecloud.com/guides', () => {
  test('@kind:keyboard @smoke Tab navigates through up to the first 10 focusable elements', async ({ page }) => {
    await page.goto('/guides')
  ...
  test('@kind:keyboard @escape-dismiss Pressing Escape closes an open dialog and restores focus', async ({ page }) => {
    await page.goto('/guides')
    // Find a candidate dialog or modal trigger; skip if absent.
    const trigger = page.locator('[aria-haspopup="dialog"], [data-modal-trigger], [aria-controls][aria-expanded]').first()
    if (await trigger.count() === 0) test.skip()
    ...
    await page.keyboard.press('Escape')
    const stillOpen = await dialog.isVisible().catch(() => false)
    expect.soft(stillOpen, 'The dialog should close when Escape is pressed').toBe(false)
  })
  ...
  test('@kind:keyboard @enter-space Pressing Enter activates focused links and buttons', async ({ page }) => {
    ...
    if (!activated) {
      // soft so SPAs that consume the click via JS don't fail —
      // a real navigation would have flipped the flag.
      expect.soft(activated, 'Pressing Enter on the focused link produced no observable navigation or effect').toBe(true)
    }
  })

  test('@kind:keyboard @no-trap Pressing Tab after the last focusable wraps around or exits the page', async ({ page }) => {
    // Find the last focusable; Tab once more; confirm focus moved
    // OR landed back on the first focusable (wrap). What we forbid
    // is staying on the same element (a focus trap outside a modal).
    await page.goto('/guides')
    ...
    expect.soft(trapped, `Pressing Tab after the last focusable should wrap or exit focus; observed transition from ${firstActiveTag} to ${secondActiveTag}`).toBe(false)
  })
})
