...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud — a11y landmarks @ https://www.spritecloud.com/devops', () => {
  test('@kind:a11y-landmarks contains exactly one main, one h1, and at least one nav', async ({ page }) => {
    await page.goto('/devops')
  ...
  test('@kind:a11y-landmarks headings follow sequential hierarchy (no skipped levels)', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/devops')
  ...
  test('@kind:a11y-landmarks duplicate landmarks each have unique accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/devops')
  ...
  test('@kind:a11y-landmarks skip-to-content link is present for keyboard and screen reader users', async ({ page }) => {
    // The first focusable should either be a skip link OR there must
    // be an in-page anchor to #main / #content / [role=main]. WCAG
    // 2.4.1 ("Bypass Blocks") requires it for keyboard / SR users.
    await page.goto('/devops')
