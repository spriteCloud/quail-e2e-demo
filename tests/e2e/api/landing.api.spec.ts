...
test.describe('Spritecloud API contract test at https://www.spritecloud.com/', () => {
  ...
  test('API smoke test: valid email body returns 2xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'test@example.com',
      },
  ...
  test('API negative test: empty form returns 4xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {},
    })
    // 4xx is the contract; 5xx means the server crashed on empty input
  ...
  test('API negative test: invalid email format returns 4xx or validation error', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'not-an-email',
      },
  ...
  test('API negative test: very large email value doesn’t cause 5xx', async ({ request }) => {
    const huge = 'a'.repeat(50_000)
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': huge,
  ...
  test('API negative test: Unicode in email doesn’t cause 5xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'café-niño-用户-🎉-test@example.com',
      },
  ...
  test('API negative test: SQL-like input doesn’t cause 5xx or leak errors', async ({ request }) => {
    // Tests the server's input sanitization. The contract: MUST NOT
    // 5xx and MUST NOT echo back unescaped fragments.
    const payload = "'; DROP TABLE users; --"
    const response = await request.get('https://www.spritecloud.com/', {
  ...
  test('API negative test: null bytes in input don’t cause 5xx', async ({ request }) => {
    const response = await request.get('https://www.spritecloud.com/', {
      form: {
        'email-2': 'value\x00malicious',
      },
  ...
  test('API negative test: burst of requests stays healthy or gets rate-limited', async ({ request }) => {
    // 10 concurrent identical requests. Either the server rate-limits
    // (429) — fine — or it serves all 10 without 5xx-ing.
    const body = {
      'email-2': 'test@example.com',
