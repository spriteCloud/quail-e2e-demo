...
test.describe.configure({ mode: 'parallel' })
test.describe('Spritecloud accessibility landmark structure — https://www.spritecloud.com/', () => {
  test('@kind:a11y-landmarks @smoke single main region, h1 heading, and navigation region', async ({ page }) => {
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @negative no interactive elements inside aria-hidden sections', async ({ page }) => {
    await page.goto('/')
    const hiddenFocusables = await page.locator('[aria-hidden="true"]').locator('a, button, input, select, textarea, [tabindex]:not([tabindex="-1"])').count()
    expect.soft(hiddenFocusables, `${hiddenFocusables} focusable elements inside aria-hidden — confusing for screen readers`).toBe(0)
  })

  test('@kind:a11y-landmarks @heading-hierarchy headings follow logical rank order without skips', async ({ page }) => {
    // Walk every heading in source order. A jump from h1 to h3 (skipping
    // h2) breaks screen-reader navigation. h2 to h4 is the same bug at
    // the next level.
    await page.goto('/')
  ...
  test('@kind:a11y-landmarks @landmark-names repeated landmark regions have unique accessible names', async ({ page }) => {
    // When ≥2 navs / asides / sections exist, each must carry an
    // accessible name (aria-label, aria-labelledby, or an inner
    // <h2-6>) so screen-reader users can distinguish them.
    await page.goto('/')
